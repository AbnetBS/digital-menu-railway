...
/* ── 1. the guest's own status UI ─────────────────────────────────────────── */
{
  ...
  pass("the bill button needs the FOOD to be served (kitchen done / no food ordered)", /const served = Boolean\(ticket\) && \(ticket!\.kitchen\.units === 0 \|\| ticket!\.kitchen\.state === "ready"\)/.test(orderStatusUi) && /served && !billRequested/.test(orderStatusUi));
  pass("while it is still cooking the guest is told when the button appears", /\{!served && !billRequested/.test(orderStatusUi) && /os_bill_after_food/.test(orderStatusUi));
  ...
  pass("the banner hides itself when there is nothing to say", /empty:hidden/.test(app) && /ticket\.phase === "drinks_only" \|\| ticket\.phase === "none"/.test(orderStatusUi));
