...
const PROMOTION_OPTIONS: Array<{ value: "none" | DailyPromotionType; label: StaffPhrase }> = [
...
const PROMOTION_LABELS: Record<DailyPromotionType, StaffPhrase> = Object.fromEntries(
  PROMOTION_OPTIONS.filter((option): option is { value: DailyPromotionType; label: StaffPhrase } => option.value !== "none").map((option) => [option.value, option.label])
) as Record<DailyPromotionType, StaffPhrase>;
...
export default function DailyBoardTab() {
  ...
  const save = async () => {
    ...
    if (r.ok) {
      ...
      alert(response.error || L("Failed to save announcement"));
    }
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-xl font-serif font-bold text-amber-100">{L("📢 Daily Board")}</h2>
          <p className="text-xs text-stone-400">
            {L("Rotating promos on the customer QR menu: specials, new items, sold-out notes, holiday greetings. Slides automatically when you have 2+.")}
          </p>
        </div>
        <div className="flex gap-2">
          ...
          >
            <Plus className="w-4 h-4" /> {L("New Announcement")}
          </button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {items.map((a) => (
          <div key={a.id} className="bg-[#2C1B17] border border-stone-800 rounded-2xl p-4 space-y-3">
            <div className="flex items-start justify-between gap-2">
              ...
              >
                {isLive(a) ? L("● LIVE") : L("Scheduled")}
              </span>
            </div>
            <div className="flex items-center gap-2 text-[11px] text-stone-500">
              <Calendar className="w-3.5 h-3.5 text-[#C9A227]" />
              <span>
                {a.startDate || L("Any")} → {a.endDate || L("Any")}
                {a.imageUrl ? L(" • 📸 image") : ""}
                {a.promotionItems ? ` • 🎁 ${L(PROMOTION_LABELS[a.promotionItems.type])}` : L(" • display only")}
              </span>
            </div>
            <div className="flex justify-end gap-2">
              <button onClick={() => setEditing(a)} className="p-2 bg-amber-500/20 text-amber-300 hover:bg-amber-500 hover:text-black rounded-lg" title={L("Edit")}>
                <Edit3 className="w-4 h-4" />
              </button>
              <button onClick={() => remove(a.id)} className="p-2 bg-rose-500/20 text-rose-300 hover:bg-rose-500 hover:text-white rounded-lg" title={L("Delete")}>
        ...
        {items.length === 0 && (
          <div className="col-span-2 bg-[#2C1B17] border border-stone-800 rounded-2xl p-10 text-center text-stone-500 text-xs">
            {L("No announcements yet. Create your first daily special above!")}
          </div>
        )}
      </div>
          ...
          <div className="bg-[#2C1B17] rounded-3xl max-w-md w-full p-6 border border-[#C9A227] space-y-4 max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between border-b border-stone-800 pb-3">
              <h3 className="font-serif font-bold text-lg text-amber-100 flex items-center gap-2">
                <Megaphone className="w-5 h-5 text-[#C9A227]" /> {editing.id ? L("Edit") : L("New")} {L("Announcement")}
              </h3>
              <button onClick={() => setEditing(null)} className="text-stone-400 hover:text-white">✕</button>
            </div>

            <div>
              <label className="block text-xs font-bold text-amber-200 mb-1">{L("Title *")}</label>
              <input
                value={editing.title || ""}
                onChange={(e) => setEditing({ ...editing, title: e.target.value })}
                placeholder={L("🔥 Today's Special")}
                className="w-full bg-[#3D2314] border border-stone-700 rounded-xl p-3 text-xs text-white"
              />
            </div>

            <div>
              <label className="block text-xs font-bold text-amber-200 mb-1">{L("Message *")}</label>
              <textarea
                rows={2}
                value={editing.description || ""}
                onChange={(e) => setEditing({ ...editing, description: e.target.value })}
                placeholder={L("20% OFF Chicken Sandwich • today only!")}
                className="w-full bg-[#3D2314] border border-stone-700 rounded-xl p-3 text-xs text-white"
              />
            </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="block text-xs font-bold text-amber-200 mb-1">{L("Start Date")}</label>
              ...
              <div>
                <label className="block text-xs font-bold text-amber-200 mb-1">{L("End Date (auto-hides)")}</label>
            ...
            <div className="bg-[#3D2314] rounded-2xl p-4 border border-stone-700 space-y-3">
              <div>
                <p className="text-xs font-bold text-amber-200 flex items-center gap-1.5"><Gift className="w-3.5 h-3.5" /> {L("Promotion")}</p>
                <p className="text-[11px] text-stone-400 mt-1">{L("Promotions use your existing menu items. Customer prices are calculated securely when the order is sent.")}</p>
              </div>
              <div>
                <label className="block text-[11px] font-bold text-stone-300 mb-1">{L("Promotion type")}</label>
                <select
                  value={promotionType}
                  onChange={(e) => {
                ...
                >
                  {PROMOTION_OPTIONS.map((option) => <option key={option.value} value={option.value}>{L(option.label)}</option>)}
                </select>
              </div>
              ...
              {promotion && (
                <>
                  <label className="flex items-center gap-2 text-[11px] font-bold text-emerald-300 cursor-pointer">
                    ...
                    {L("Promotion is active and can be ordered")}
                  </label>
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      <label className="block text-[11px] font-bold text-stone-300 mb-1">{L("Start Time (optional)")}</label>
                    ...
                    <div>
                      <label className="block text-[11px] font-bold text-stone-300 mb-1">{L("End Time (optional)")}</label>
                  ...
                  {(promotion.type === "special_price" || promotion.type === "combo") && (
                    <div>
                      <label className="block text-[11px] font-bold text-stone-300 mb-1">{promotion.type === "combo" ? L("Combo price (ETB)") : L("Special price per item (ETB)")}</label>
                  ...
                  {promotion.type === "percentage_discount" && (
                    <div>
                      <label className="block text-[11px] font-bold text-stone-300 mb-1">{L("Discount (%)")}</label>
                  ...
                  {promotion.type === "fixed_amount_discount" && (
                    <div>
                      <label className="block text-[11px] font-bold text-stone-300 mb-1">{L("Discount amount (ETB)")}</label>
                  ...
                  {promotion.type === "buy_x_get_y_discounted" && (
                    <div>
                      <label className="block text-[11px] font-bold text-stone-300 mb-1">{L("Get item discount (%)")}</label>
                  ...
                  <div className="space-y-2">
                    <p className="text-[11px] font-bold text-stone-300">{needsBuyGetRoles ? L("Buy and get items") : promotion.type === "combo" ? L("Bundle items") : L("Menu item")}</p>
                      ...
                      <div key={index} className="rounded-xl border border-stone-700 bg-[#2C1B17] p-3 space-y-2">
                        <div className="flex gap-2">
                          <select value={promotionItem.menuItemId || ""} onChange={(e) => updatePromotionItem(index, { menuItemId: Number(e.target.value) })} className="min-w-0 flex-1 bg-[#3D2314] border border-stone-700 rounded-lg px-2 py-2 text-xs text-white">
                            <option value="" disabled>{L("Select menu item")}</option>
                            {menuItems.map((m) => <option key={m.id} value={m.id}>{m.name} • {m.price} ETB</option>)}
                          </select>
                          {canAddPromotionItem && (
                            <button type="button" onClick={() => updatePromotion({ items: promotion.items.filter((_, itemIndex) => itemIndex !== index) })} className="w-8 h-8 shrink-0 rounded-lg bg-rose-500/15 text-rose-300 hover:bg-rose-500 hover:text-white flex items-center justify-center" aria-label={L("Remove promotion item")}>
                          ...
                          {needsBuyGetRoles && (
                            <label className="flex items-center gap-2 text-[11px] font-bold text-amber-200">
                              ...
                              <select value={promotionItem.role || "buy"} onChange={(e) => updatePromotionItem(index, { role: e.target.value as "buy" | "get" })} className="bg-[#3D2314] border border-stone-700 rounded-lg px-2 py-1.5 text-xs text-white">
                                <option value="buy">{L("Buy / pay normal price")}</option>
                                <option value="get">{L("Get promotional item")}</option>
                              </select>
                            </label>
                          )}
                        </div>
            ...
            <div className="bg-[#3D2314] rounded-2xl p-4 border border-stone-700 space-y-2">
              <p className="text-xs font-bold text-amber-200">{L("Image (optional)")}</p>
              {editing.imageUrl && <img src={editing.imageUrl} alt={L("preview")} className="w-full h-28 object-cover rounded-xl border border-stone-600" />}
              <label className="flex items-center justify-center gap-2 w-full bg-[#C9A227] hover:bg-amber-400 text-[#2C1B17] font-extrabold text-xs py-2.5 rounded-xl cursor-pointer">
                <Upload className="w-4 h-4" /> {editing.imageUrl ? L("Change Photo") : L("Upload Photo")}
                <input type="file" accept="image/*" className="hidden" onChange={(e) => handlePhoto(e.target.files?.[0])} />
              </label>
            </div>

            <div>
              <label className="block text-xs font-bold text-amber-200 mb-1">{L("Priority (lower shows first)")}</label>
            ...
            <button onClick={save} disabled={saving || !editing.title} className="w-full bg-gradient-to-r from-[#C9A227] to-amber-500 text-[#2C1B17] font-black text-sm uppercase py-3.5 rounded-xl disabled:opacity-40">
              {saving ? L("Saving...") : L("Post To Daily Board")}
            </button>
          </div>
        </div>
      )}
