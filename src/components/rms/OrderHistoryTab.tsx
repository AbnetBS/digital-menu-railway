...
export default function OrderHistoryTab() {
  ...
  // Server audit lines read "Quantity changed • Cappuccino • yeshi" and
  // "pending waiter → confirmed": translate the known words piece by piece,
  // leaving item names, staff names and order numbers exactly as they are.
  const trAudit = (text: string | null | undefined) =>
    String(text ?? "")
      .split(" • ")
    ...
    <div className="space-y-5">
      {expired && (
        <div className="bg-rose-900/60 border border-rose-500 text-rose-200 text-xs p-3 rounded-xl font-bold">
          {L("Your admin session ended. Reload the page and log in again to see fresh history.")}
        </div>
      )}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          <h2 className="text-xl font-serif font-bold text-amber-100">{L("Order History ({length})", { length: filtered.length })}</h2>
          ...
          <p className="text-xs text-stone-400">
            {L("Newest finished bills of the last 30 days (today plus the 29 days before it) with audit trail • older bills are still stored, they just slide out of this window • search by date, table, waiter, outdoor note or status.")}
          </p>
        </div>
        <div className="flex gap-2 self-start">
          <button
            ...
            title={L("Clear receipt photos older than 30 days to free storage")}
          >
            <Trash2 className="w-4 h-4" /> {L("Clean Old Receipts")}
          </button>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        <div className="relative">
          ...
          <input
            ...
            placeholder={L("Search table, waiter, outdoor note, date...")}
        ...
        <select value={statusFilter} onChange={(e) => setStatusFilter(e.target.value)} className="bg-[#2C1B17] border border-stone-700 rounded-xl p-2.5 text-xs text-white">
          <option value="all">{L("All Statuses")}</option>
          <option value="done">{L("Done")}</option>
          <option value="edited_printed">{L("Edited & Printed")}</option>
          <option value="edited_cancelled">{L("Edited & Cancelled")}</option>
      ...
      {filtered.length === 0 ? (
        <div className="p-10 bg-[#2C1B17] rounded-2xl border border-stone-800 text-center text-stone-500 text-xs">
          {L("No orders match your search. Closed and paid bills appear here automatically.")}
        </div>
      ) : (
            ...
            <div key={o.id} className="bg-[#2C1B17] rounded-2xl border border-stone-800 p-4 space-y-3">
                  ...
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    {o.orderType === "outdoor" && (
                      <span className="inline-block text-[10px] font-black px-2.5 py-1 rounded-full uppercase bg-violet-500/20 text-violet-300 border border-violet-500/40">
                        {/^GROUP \d+$/i.test(String(o.tableName || "")) ? L("👥 Group") : L("Outdoor")}
                      </span>
                    )}
                  </div>
                  <p className="text-[10px] text-stone-500">
                    {L("{time} • by", { time: fmtTime(o) })} {o.createdBy || L("staff")}
                  </p>
                  {/* Group 8: when the order actually ARRIVED, not just when it closed. */}
                  <p className="text-[10px] text-stone-500">
                    {L("🕒 arrived {dateTime}", { dateTime: formatDateTime(o.createdAt) })}
                  </p>
                ...
                <div className="text-right flex flex-col items-end gap-1.5">
                  <div className="flex items-center gap-1.5 flex-wrap justify-end">
                    ...
                    <button
                      ...
                      title={L("Delete this order record")}
                    >
              ...
              {(o.status === "paid" || o.status === "completed") && (
                <div className="flex items-center justify-end bg-black/30 rounded-xl px-3 py-2">
                  ...
                  >
                    <ImageIcon className="w-3 h-3" /> {L("Receipt")}
                  </button>
                </div>
              )}
              ...
              {(o.historyChangeSummary || (o.auditTrail || []).length > 0) && (
                <div className="bg-black/25 border border-stone-800 rounded-xl p-3 space-y-2">
                  <div className="flex items-center justify-between gap-2">
                    <p className="text-[10px] font-extrabold uppercase tracking-wider text-amber-200">{L("Audit Trail")}</p>
                  ...
                  <div className="space-y-2 max-h-44 overflow-y-auto pr-1">
                      ...
                      <div key={event.id} className="rounded-lg bg-[#241714] border border-stone-800 px-2.5 py-2">
                        <div className="flex items-start justify-between gap-3">
                          <div className="min-w-0">
                            <p className="text-[11px] font-black text-stone-200">{trAudit(event.label)}</p>
                            {event.detail && <p className="text-[10px] text-stone-400 mt-0.5 break-words">{trAudit(event.detail)}</p>}
                          </div>
                    ...
                    {(o.auditTrail || []).length === 0 && (
                      <p className="text-[11px] text-stone-500">{L("Older record with no detailed audit entries saved yet.")}</p>
                    )}
                  </div>
                </div>
              )}
