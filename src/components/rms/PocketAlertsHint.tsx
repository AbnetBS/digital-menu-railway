  ...
  return (
    <div className="bg-sky-950/60 border border-sky-700/70 rounded-2xl p-4 flex items-start gap-3">
      ...
      <div className="flex-1 space-y-1">
        <p className="text-xs font-bold text-sky-200">{L("📱 Make your iPhone ring in your pocket")}</p>
        <p className="text-[11px] text-sky-300/80 leading-relaxed">
          {Lr("Tap <b>Share</b> → <b>Add to Home Screen</b>, then always open Fana from the home screen icon. Apple only allows pocket notifications for installed apps. (Android phones work automatically.)", { b: (s) => <strong>{s}</strong> })}
        </p>
      </div>
      <button onClick={dismiss} className="p-1.5 rounded-lg bg-white/10 text-sky-200 shrink-0" title={L("Got it")}>
        <X className="w-4 h-4" />
      </button>
    </div>
  );
